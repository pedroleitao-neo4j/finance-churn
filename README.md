...
This project implements a churn prediction system for financial data using Neo4j's Graph Data Science (GDS) library. It leverages graph-based features (embeddings and centrality) alongside traditional demographic data to identify active users at risk of churning. The dataset used is sourced from Kaggle's [computingvictor/transactions-fraud-datasets](https://www.kaggle.com/datasets/computingvictor/transactions-fraud-datasets), this was chosen as it includes enough structure (merchants, transactions, cards and users) to derive an interesting graph architecture. However it does not include a specific churn label, so we engineered one based on user inactivity.
